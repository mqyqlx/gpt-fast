...
#model_name="DynamicDense";use_dcmha=""; q_chunk_sizes=("2048")
...
for model_size in "${model_sizes[@]}"; do
    for batch_size in "${batch_sizes[@]}"; do
        for q_chunk_size in "${q_chunk_sizes[@]}"; do
            ...
            time python train.py --model_name ${model_name} --model_size ${model_size} ${use_dcmha} --query_chunk_size ${q_chunk_size} --batch_size ${batch_size} ${compile} > logs/training_${model_name}_${model_size}_qchunk${q_chunk_size}_batchsize${batch_size}_compiled.log 2>&1 
